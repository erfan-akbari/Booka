...
function Tab() {
    ...
    return (
        <div className='py-10 transition-all'>
            <div class="grid grid-cols-3 gap-5 font-bold">
                <button
                    onClick={() => setTypeShowContent('معرفی')}
                    class={`p-4 rounded  ${typeShowContent === 'معرفی' ? 'bg-gradient-to-b from-red-500 to-rose-700 text-white' : 'text-rose-700 bg-white'}  shadow-md flex items-center justify-center`}
                > معرفی کتاب
                </button>
                <button
                    onClick={() => setTypeShowContent('نظرات')}
                    class={`p-4 rounded ${typeShowContent === 'نظرات' ? 'bg-gradient-to-b from-red-500 to-rose-700 text-white' : 'text-rose-700 bg-white'} shadow-md flex items-center justify-center`}
                >
                    نظرات
                </button>
                <button
                    onClick={() => setTypeShowContent('مشخصات')}
                    class={`p-4 rounded ${typeShowContent === 'مشخصات' ? 'bg-gradient-to-b from-red-500 to-rose-700 text-white' : 'text-rose-700 bg-white'} shadow-md flex items-center justify-center`}
                >
                    مشخصات
                </button>
            </div>
            {typeShowContent === 'معرفی' && (
                <div class="shadow-xl border border-gray-100 font-semibold p-8 rounded text-gray-500 bg-white mt-6">
                    توضیحات محصول
                ...
                <div class="shadow-xl border border-gray-100 font-semibold p-8 rounded text-gray-500 bg-white mt-6">
                    ...
                    <div class="max-w-2xl mx-auto px-4">
                        <div class="flex justify-between items-center mb-6">
                            <h2 class="text-lg lg:text-2xl font-bold text-gray-900 dark:text-white">کامنت ها (20)</h2>
                        </div>
                        <form class="mb-6">
                            <div class="py-2 px-4 mb-4 bg-white rounded-lg rounded-t-lg border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                                <label for="comment" class="sr-only">کامنت شما</label>
                                <textarea id="comment" rows="6"
                                    class="px-0 w-full text-sm text-gray-900 border-0 focus:ring-0 focus:outline-none dark:text-white dark:placeholder-gray-400 dark:bg-gray-800"
                                    placeholder="Write a comment..." required></textarea>
                            </div>
                            <button type="submit"
                                class="inline-flex items-center py-2.5 px-4 text-xs font-medium text-center text-white bg-rose-700 rounded-lg hover:bg-rose-500 ">
                                ارسال
                            </button>
                        </form>
                    </div>

                    <section class="relative flex items-center justify-center min-h-max py-5 antialiased bg-gray-100 min-w-screen">
